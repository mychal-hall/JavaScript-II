...
let ticketPriceTotal = [];
    ...
    }, 0); // <-- that little zero makes it tick.
...
// Problem 1 Well the power hungry director forgot to tell the city about the 5k. So none of the route has been cleared for use. He needs to email all the runners and let them know that the 5k has been moved to a farm outside of town.

let emailList = [];
    runners.forEach(function (runner) {
        return emailList.push(runner.email);
    });

console.log(emailList)

// Problem 2 No one is sure why this person is in charge any more, but it's too late now. He needs a list of people that donated less than 40 units of generic currency. So he can bully them.
let lessMoney = [];
    runners.filter(function (runner) {
        if (runner.donation < '40') {
          lessMoney.push(runner);
        }
    });
...
// Problem 3 The bullying didn't work. Naturally. However, in a swing the other direction... The director of the run has decided to file a massive lawsuit against companies that each runner works at... he is suing for emational damages caused by their low donations, stemming from the fact that they are underpaid.
let companyName = [];
    runners.forEach(function (runner) {
        return companyName.push(runner.first_name + " works at " + runner.company_name);
    });
